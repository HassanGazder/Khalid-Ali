  ...
  return (
    <section id="skills" className="relative overflow-hidden py-20 sm:py-28 lg:py-32">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-4 max-w-3xl font-serif text-3xl font-bold text-zinc-950 sm:text-4xl lg:text-5xl">
        ...
        <div className="mt-14 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {SKILLS.map((skill, index) => (
            <motion.div
              ...
              className="group flex min-h-20 items-center gap-4 border border-black/10 bg-white/70 px-5 py-4 transition-[border-color,background-color,transform] duration-300 hover:-translate-y-1 hover:border-gold/60 hover:bg-white"
            >
              <span className="font-mono text-[10px] text-crimson">{String(index + 1).padStart(2, "0")}</span>
              <p className="text-sm font-medium text-zinc-700 transition-colors group-hover:text-zinc-950">{skill}</p>
            </motion.div>
          ))}
        </div>
      </div>
